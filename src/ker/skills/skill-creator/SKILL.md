...
**Installing into Ker:** To persist a skill so the current agent can discover it, use `skill(action="install", name="<skill-name>", content="<SKILL.md content>")`. This stores the skill at `.ker/agents/{agent}/skills/{name}/SKILL.md` and makes it immediately available. Do NOT write skill files to paths manually — use the install action to ensure correct placement.
